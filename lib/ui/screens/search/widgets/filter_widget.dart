...
class FilterWidget extends StatelessWidget {
      ...
      child: Column(
        children: [
          ...
          Visibility(
            ...
            child: BlocBuilder<SearchCubit, List<RadioBtn>>(
                builder: (ctx, state) {
              return Flexible(
  ...
  void _onItemClick(BuildContext context, int index) {
    if (filterType == FilterType.Type) {
      context
          .read<SearchCubit>()
          .emitActivatedState(index);
    } else if (filterType == FilterType.Location) {
      context
          .read<DistrictCubit>()
          .generateDistricts(navigatorKey.currentState!.context.read<SearchDataCubit>().cities[index]);
